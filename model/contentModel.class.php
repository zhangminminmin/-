...
class contentModel extends commonMod {
	...
	//分享计数
    public function shares_content($aid) {
		$sql = "update {$this->model->pre}content set share=share+1 where aid=".$aid;
    ...
    public function text_info($aid) {
        return $this->model->table('img')->where('id=' . $aid)->find();
    }

    public function text_views_content($aid, $views) {
        $data['click'] = $views + 1;
        $condition['id'] = $aid;
        $this->model->table('img')->data($data)->where($condition)->update();
    }
