...
class goodCourseSortMod extends commonMod {
    ...
    public function editSave()
    {
        ...
        $info = $this->data_getinfo("good_course_category", " id = '" . $input['id'] . "'");
        if (empty($info)) {
            $this->ajaxReturn(202, "此分类不存在或者已经被删除");
        }

        $categoryInfo = $this->data_getinfo("good_course_category", " id != '" . $input['id'] . "' and pid = '" . $info['pid'] . "' and name = '" . $input['name'] . "'");
        ...
        $editCategory = $this->data_edit("good_course_category", $data, " id='" . $input['id'] . "'");
    ...
    public function addSave()
    {
        ...
        // 检查是否存在
        $info = $this->data_getinfo("good_course_category", " pid = '" . $pid . "' and  name = '" . $input['name'] . "'");
        if (!empty($info)) {
        ...
        $addCategory = $this->data_add("good_course_category", $data);
        $list = $this->newCategorys();
    ...
    public function delCategory()
    {
        ...
        // 查看下面是否有分类
        $cateInfo = $this->data_getinfo("good_course_category", " pid = '" . $input['id'] . "'");
        if (! empty($cateInfo)) {
            $this->ajaxReturn(202, "此分类下面存在子分类 请先删除子分类");
        }

        $del = $this->data_del("good_course_category", " id = '" . $input['id'] . "'");
        $list = $this->newCategorys();
        if (empty($del)) {
