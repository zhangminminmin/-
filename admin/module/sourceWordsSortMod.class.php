...
class sourceWordsSortMod extends commonMod {
    ...
    {
        $data = model("u")->data_list("source_word_sort", "where id > 0 and pid = 0", " order by id desc");
        if (! empty($data)) {
    ...
    public function addSave()
    {
        ...
        $info = $this->data_getinfo("source_word_sort", " name = '" . $input['name'] . "'");
        if (! empty($info)) {
        ...
        $addsort = $this->data_add("source_word_sort", $data);
    ...
    public function editSave()
    {
        ...
        $categoryInfo = $this->data_getinfo("source_word_sort", " id != '" . $input['id'] . "' and pid = 0 and name = '" . $input['name'] . "'");
    ...
    public function delCategory()
    {
        ...
        if (! empty($cateInfo)) {
            $this->ajaxReturn(202, "此分类下面存在子分类 无法删除");
        }
