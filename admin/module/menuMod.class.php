...
class menuMod extends commonMod
{
       ...
       if(!empty($tree)){
           foreach ($tree as $value) {
                if(empty($value['admin_content'])){
                    ...
                    if (!in_array($value['cid'], array(83,84,85,86,87))) {//跳转栏目 去除修改 @author puxiann 2014-5-24
    ...
    public function member() {
        $this->list=model('menu')->admin_menu(124);
        $this->display();
    }
	*/
    ...
    public function weixin() {
        $this->list = model('menu')->admin_menu(100);
        $this->display();
    }

    // 微信
    public function wz() {
        $this->list = model('menu')->admin_menu(130);
        $this->display();
    }

    // 留言
    public function message() {
        $this->list = model('menu')->admin_menu(140);
        $this->display();
    }

    public function hy() {
        $this->list = model('menu')->admin_menu(150);
        $this->display();
    }
