...
class buyUserVipMod extends commonMod {
    ...
    {
        $sql = "SELECT o.*, u.mobile FROM {$this->model->pre}user_order AS o ".
    ...
    {
        $sql = "SELECT COUNT(1) AS num FROM {$this->model->pre}user_order AS o ".
               "LEFT JOIN {$this->model->pre}user AS u ON o.user_id = u.id ".
               "{$where}";
